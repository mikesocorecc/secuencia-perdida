<template>
  <div class="">
    ...
    <TransitionGroup
      name="list"
      tag="div"
      class="flex relative md:static items-center justify-center my-10"
    >
      <template v-for="(number, index) in sequence" :key="index">
        <h2
          ...
          :class="getMissingNumberClasses(number)"
          :style="{ transitionDuration: index * 0.1 + 's' }"
        >
          {{ number ?? selectedOption }}
          <OptionsValues
            v-if="number == null && !isShowingResult"
            :options="options"
        ...
        <img
          ...
          class="grow-0 mx-2 w-6 md:w-14 lg:w-16 lg:mx-5"
